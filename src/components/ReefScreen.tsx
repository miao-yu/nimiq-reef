...
import { COLLECTION_ICON, REEFS_ICON, SideRail } from './SideRail';
...
import styles from './ReefScreen.module.css';
 ...
 * The reef screen.
 *
 * `signedIn` comes from the server, which already knows — the session is a
 * cookie it just read. Without it this component starts with reef === null and
 * cannot tell "signed out" from "not asked yet", so every navigation back from
 * the collection or the community flashed the sign-in gate before the reef
 * arrived. The server settles that before a single pixel is drawn.
 */
export function ReefScreen({ signedIn }: { signedIn: boolean }) {
  const { t } = useLocale();
  ...
  /**
   * Whether /api/reef has answered yet — not whether it found anything.
   *
   * Distinguishing "no reef" from "no answer yet" is the whole fix: the gate
   * may only appear once one of those is actually true.
   */
  ...
  return (
    ...
    >
      ...
      {reef ? (
        ...
        /*
         * Known to be signed in, reef not back yet. Show neither face.
         *
         * The water is already on screen behind this, so an empty moment reads
         * as a reef loading. Showing the gate here read as being signed out,
         * which was worse than showing nothing and was wrong besides.
         */
        null
      ) : (
